 ...
 * Function signatures modified anticipating its use by a class that can perform
...
template <typename T>
inline void
MultiBspline<T>::evaluate_vgl_impl(T x, T y, T z, T *restrict vals,
                                   T *restrict grads, T *restrict lapl,
                                   int first, int last, size_t out_offset) const
{
...
template <typename T>
inline void
MultiBspline<T>::evaluate_vgh_impl(T x, T y, T z, T *restrict vals,
