...
{
  using SingleParticlePos = typename PS::SingleParticlePos;
  ...
  ParticleSet::ParticleLayout PrimCell(ref_.getLattice());
  auto& lattice = const_cast<SimulationCell::Lattice&>(ref_.getLattice());
  lattice.set(dot(tmat, PrimCell.R));
  int natoms    = ref_.getTotalNum();
  int numCopies = std::abs(det(tmat));
  ParticleSet::ParticlePos primPos(ref_.R);
  ParticleSet::ParticleIndex primTypes(ref_.GroupID);
      ...
      for (int i1 = -maxCopies; i1 <= maxCopies; i1++)
        for (int i2 = -maxCopies; i2 <= maxCopies; i2++)
          for (int iat = 0; iat < primPos.size(); iat++)
          {
            if (primTypes[iat] != ns)
              continue;
            SingleParticlePos uPrim = primPos[iat];
            for (int i = 0; i < 3; i++)
              uPrim[i] -= std::floor(uPrim[i]);
            SingleParticlePos r = PrimCell.toCart(uPrim) + (double)i0 * PrimCell.a(0) + (double)i1 * PrimCell.a(1) +
                (double)i2 * PrimCell.a(2);
            SingleParticlePos uSuper = ref_.getLattice().toUnit(r);
