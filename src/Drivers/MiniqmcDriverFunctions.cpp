...
template<>
void 
MiniqmcDriverFunctions<Devices::CPU>::batched_thread_main(const int ip,
                                                  const int team_size,
                                                  ...
                                                  const SPOSet* spo_main)
{
  ...
  Movers<Devices::CPU> movers(myPrimes, ions, mq_opt.pack_size);

  // create a spo view in each Mover
  movers.buildViews(mq_opt.useRef,
		    spo_main,
		    team_size,
		    member_id);

  movers.buildWaveFunctions(mq_opt.useRef,
			    mq_opt.enableJ3);

  // initial update
  // no problem with execution policies C++17
  std::for_each(movers.elss.begin(),
		movers.elss.end(),
		[](ParticleSet& els){els.update();});
