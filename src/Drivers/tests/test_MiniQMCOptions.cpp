...
TEST_CASE("MiniQMCOptions Read", "[Application]")
{
  ...
  const int argc = opt_strs.size();

  std::vector<char*> option_ptrs;
  option_ptrs.reserve(argc + 1);
  std::transform(begin(opt_strs), end(opt_strs), std::back_inserter(option_ptrs), [](std::string& s) {
  ...
  std::for_each(begin(option_ptrs), end(option_ptrs), [](char* c) { delete[] c; });
