...
struct MBspline_create_cuda<double, double>
{
   ...
   struct einspline_cpu_coefs_create<float>
    {
	void operator()(typename bspline_traits<Devices::CPU, float, 3>::SplineType*& spline, int num_splines)
	    {
    einspline_create_multi_UBspline_3d_s_coefs<Devices::CPU>(spline, spline->x_grid.num+3, spline->y_grid.num+3,
   ...
   struct einspline_cpu_coefs_create<double>
    {
	void operator()(typename bspline_traits<Devices::CPU, double, 3>::SplineType*& spline, int num_splines)
	    {
    einspline_create_multi_UBspline_3d_d_coefs<Devices::CPU>(spline, spline->x_grid.num+3, spline->y_grid.num+3,
						spline->z_grid.num+3, num_splines);
	    }
    };
						     ...
						     typename bspline_traits<Devices::CUDA, float, 3>::SplineType*& target_spline, double dummyT, float dummyDT)
{
  target_spline = create_multi_UBspline_3d_s_cuda_conv(spline);
    ...
    template<typename T, typename DT>
void Allocator<Devices::CUDA>::createMultiBspline(  aligned_vector<typename bspline_traits<Devices::CPU, T, 3>::SplineType*>& cpu_splines,
						     typename bspline_traits<Devices::CUDA, DT, 3>::SplineType*& target_spline, T dummyT, DT dummyDT)
{
    int num_splines = std::accumulate(cpu_splines.begin(), cpu_splines.end(), 0,
				      [](int a,
					 typename bspline_traits<Devices::CPU, T, 3>::SplineType* spl)
				      {
    ...
    cpu_allocator.allocateMultiBspline(spline, cpu_spline->x_grid,
				       cpu_spline->y_grid,
				       cpu_spline->z_grid,
				       cpu_spline->xBC,
				       cpu_spline->yBC,
				       cpu_spline->zBC,
				       num_splines);
    int grid_size = (cpu_spline->x_grid.num + cpu_spline->y_grid.num + cpu_spline->z_grid.num) * sizeof(T);
    
  for(int i = 0; i < num_splines; ++i)
      std::memcpy(&spline->coefs[sizeof(T)*grid_size*i],(cpu_splines[i]->coefs),grid_size);
...
template class Allocator<qmcplusplus::Devices::CUDA>;
    template void Allocator<qmcplusplus::Devices::CUDA>::createMultiBspline(  aligned_vector<typename bspline_traits<Devices::CPU, double, 3>::SplineType*>& cpu_splines,
										      typename bspline_traits<Devices::CUDA, double, 3>::SplineType*& target_spline, double dummyT, double dummyDT);
    
    template void Allocator<qmcplusplus::Devices::CUDA>::createMultiBspline(  aligned_vector<typename bspline_traits<Devices::CPU, double, 3>::SplineType*>& cpu_splines,
										      typename bspline_traits<Devices::CUDA, float, 3>::SplineType*& target_spline, double dummyT, float dummyDT);

        template void Allocator<qmcplusplus::Devices::CUDA>::createMultiBspline(  aligned_vector<typename bspline_traits<Devices::CPU, float, 3>::SplineType*>& cpu_splines,
