...
__global__ static void test_kernel(int N)
{
...
__global__ static void test_kernel_with_spline(const double* coefs, int N)
{
  int block = blockIdx.x;
  int thr   = threadIdx.x;
  int ir    = blockIdx.y;
...
extern "C" void launch_test_kernel(int N)
{
...
extern "C" void launch_test_kernel_with_spline(multi_UBspline_3d_d<Devices::CUDA>* spline, int N)
{
  dim3 dimBlock(SPLINE_BLOCK_SIZE);
  dim3 dimGrid(7, N);
