...
template<typename T>
inline void
evaluate_v(const typename bspline_traits<T, 3>::SplineType* restrict spline_m, int ix, int iy,
           int iz, const T a[4], const T b[4], const T c[4], T* restrict vals, size_t num_splines)
{
...
template<typename T>
inline void
evaluate_v_v2(const typename bspline_traits<T, 3>::SplineType* restrict spline_m, int ix, int iy,
              int iz, const T a[4], const T b[4], const T c[4], T* restrict vals, int first, int ind)
{
  const intptr_t xs = spline_m->x_stride;
  const intptr_t ys = spline_m->y_stride;
  const intptr_t zs = spline_m->z_stride;
  ...
  for (size_t i = 0; i < 4; i++)
    for (size_t j = 0; j < 4; j++)
    {
      ...
      val += a[i] * b[j] *
          (c[0] * coefs[ind] + c[1] * coefs[ind + zs] + c[2] * coefs[ind + 2 * zs] +
           c[3] * coefs[ind + 3 * zs]);
    }
  vals[ind] = val;
}
...
template<typename T>
inline void evaluate_vgl(const typename bspline_traits<T, 3>::SplineType* restrict spline_m, int ix,
                         int iy, int iz, const T a[4], const T b[4], const T c[4], const T da[4],
                         const T db[4], const T dc[4], const T d2a[4], const T d2b[4], const T d2c[4],
...
template<typename T>
inline void evaluate_vgh(const typename bspline_traits<T, 3>::SplineType* restrict spline_m, int ix,
                         int iy, int iz, const T a[4], const T b[4], const T c[4], const T da[4],
                         const T db[4], const T dc[4], const T d2a[4], const T d2b[4], const T d2c[4],
...
#else
#pragma omp simd aligned(coefs, coefszs, coefs2zs, coefs3zs, vals, gx, gy, gz, hxx, hyy, hzz, hxy, \
                         hxz, hyz)
#endif
      for (int n = 0; n < num_splines; n++)
      {
...
template<typename T>
inline void
evaluate_vgh_v2(const typename bspline_traits<T, 3>::SplineType* restrict spline_m, int ix, int iy,
                int iz, const T a[4], const T b[4], const T c[4], const T da[4], const T db[4],
                const T dc[4], const T d2a[4], const T d2b[4], const T d2c[4],
                T* restrict val_grad_hess, size_t out_offset, const int first, const int ind)
{
  const intptr_t xs = spline_m->x_stride;
  const intptr_t ys = spline_m->y_stride;
  const intptr_t zs = spline_m->z_stride;
  ...
  val_grad_hess[ind + 1 * out_offset] = gx * dxInv;
  val_grad_hess[ind + 2 * out_offset] = gy * dyInv;
  val_grad_hess[ind + 3 * out_offset] = gz * dzInv;
  val_grad_hess[ind + 4 * out_offset] = hxx * dxInv * dxInv;
  val_grad_hess[ind + 5 * out_offset] = hxy * dxInv * dyInv;
  val_grad_hess[ind + 6 * out_offset] = hxz * dxInv * dzInv;
