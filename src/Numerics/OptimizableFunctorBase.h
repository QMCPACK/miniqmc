 ...
 * @brief Define a base class for one-dimensional functions with optimizable
 * variables
 */
 ...
 * Functor in qmcpack denotes any function which returns a value at a point,
 * e.g.,
 * GTO, STO, one-dimensional splines etc. OptimizableFunctorBase is introduced
 * for
 * optimizations. The virtual functions are intended for non-critical operations
 * that
 * are executed infrequently during optimizations.
 *
 * This class handles myVars of opt_variables_type (Optimize/VariableSet.h). A
 * derived class
 * can insert any number of variables it handles during optimizations, by
 * calling
 * myVars.insert(name,value);
 * Unlike VarList which uses map, VariableSet is serialized in that the internal
 * order is according
 * to insert calls.
 */
