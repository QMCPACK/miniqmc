...
void caxpy(const int &n, const std::complex<float> &da,
           const std::complex<float> *dx, const int &incx,
           std::complex<float> *dy, const int &incy);

void daxpy(const int &n, const double &da, const double *dx, const int &incx,
           double *dy, const int &incy);

void saxpy(const int &n, const float &da, const float *dx, const int &incx,
           float *dy, const int &incy);

void zaxpy(const int &n, const std::complex<double> &da,
...
void ssymv(const char &uplo, const int &n, const float &alpha, const float &a,
...
void dzgemm(const char &, const char &, const int &, const int &, const int &,
            const std::complex<double> &, const double *, const int &,
            const std::complex<double> *, const int &,
            const std::complex<double> &, std::complex<double> *, const int &);

void scgemm(const char &, const char &, const int &, const int &, const int &,
            const std::complex<float> &, const float *, const int &,
            const std::complex<float> *, const int &,
            const std::complex<float> &, std::complex<float> *, const int &);

void dzgemv(const char &trans, const int &nr, const int &nc,
            const std::complex<double> &alpha, const double *amat,
            const int &lda, const std::complex<double> *bv, const int &incx,
            const std::complex<double> &beta, std::complex<double> *cv,
            const int &incy);
...
void sgeev(char *JOBVL, char *JOBVR, int *N, float *A, int *LDA, float *ALPHAR,
           float *ALPHAI, float *VL, int *LDVL, float *VR, int *LDVR,
           float *WORK, int *LWORK, int *INFO);

void dggev(char *JOBVL, char *JOBVR, int *N, double *A, int *LDA, double *B,
...
void zheevr(char &JOBZ, char &RANGE, char &UPLO, int &N,
...
void zhegvx(int &, char &JOBZ, char &RANGE, char &UPLO, int &N,
            ...
            int &LDB, double &VL, double &VU, int &IL, int &IU, double &ABSTOL,
            int &M, double *W, std::complex<double> *Z, int &LDZ,
            std::complex<double> *WORK, int &LWORK, double *RWORK, int *IWORK,
            int *IFAIL, int &INFO);

void zgeqrf(const int *M, const int *N, std::complex<double> *A, const int *LDA,
            std::complex<double> *TAU, std::complex<double> *WORK,
            const int *LWORK, int *INFO);

void zungqr(const int *M, const int *N, const int *K, std::complex<double> *A,
...
void dgghrd(const char *COMPQ, const char *COMPZ, const int *N, const int *ILO,
            const int *IHI, double *A, const int *LDA, double *B,
            const int *LDB, double *Q, const int *LDQ, double *Z,
            const int *LDZ, int *INFO);

void dhgeqz(const char *JOB, const char *COMPQ, const char *COMPZ, const int *N,
            const int *ILO, const int *IHI, double *H, const int *LDH,
            double *T, const int *LDT, double *ALPHAR, double *ALPHAI,
            double *BETA, double *Q, const int *LDQ, double *Z, const int *LDZ,
            double *WORK, int *LWORK, int *INFO);

void dtgexc(const bool *WANTQ, const bool *WANTZ, const int *N, double *A,
            const int *LDA, double *B, const int *LDB, double *Q,
            const int *LDQ, double *Z, const int *LDZ, int *IFST, int *ILST,
            double *WORK, int *LWORK, int *INFO);

void dtgevc(const char *SIDE, const char *HOWMNY, const bool *SELECT,
            const int *N, double *S, const int *LDS, double *P, const int *LDP,
