...
__global__ static void eval_multi_multi_UBspline_3d_s_kernel(float* pos, float3 drInv,
                                                             const float* coefs, float* vals,
                                                             uint3 dim, uint3 strides, int N)
{
  ...
  if (thr < 4)
  {
    a[thr] = Acuda[4 * thr + 0] * tp[0].x + Acuda[4 * thr + 1] * tp[0].y +
        Acuda[4 * thr + 2] * tp[0].z + Acuda[4 * thr + 3] * tp[0].w;
    b[thr] = Acuda[4 * thr + 0] * tp[1].x + Acuda[4 * thr + 1] * tp[1].y +
        Acuda[4 * thr + 2] * tp[1].z + Acuda[4 * thr + 3] * tp[1].w;
    c[thr] = Acuda[4 * thr + 0] * tp[2].x + Acuda[4 * thr + 1] * tp[2].y +
        Acuda[4 * thr + 2] * tp[2].z + Acuda[4 * thr + 3] * tp[2].w;
  }
  __syncthreads();
  int i = (thr >> 4) & 3;
  int j = (thr >> 2) & 3;
  ...
  if (off < N)
  {
    ...
    {
      const float* base =
          coefs + (index.x + i) * strides.x + (index.y) * strides.y + index.z * strides.z + off;
...
eval_multi_multi_UBspline_3d_s_sign_kernel(float* pos, float* sign, float3 drInv,
                                           const float* coefs, float* vals, uint3 dim,
                                           uint3 strides, int N, int spline_offset)
{
  int block = blockIdx.x;
  int thr   = threadIdx.x;
  int ir    = blockIdx.y;
...
__global__ static void
eval_multi_multi_UBspline_3d_s_vgh_kernel(float* pos, float3 drInv, const float* coefs, float* vals,
                                          float* grads, float* hess, uint3 dim, uint3 strides, int N)
{
  int block = blockIdx.x;
  int thr   = threadIdx.x;
  int ir    = blockIdx.y;
  ...
  __syncthreads();
  float v = 0.0, g0 = 0.0, g1 = 0.0, g2 = 0.0, h00 = 0.0, h01 = 0.0, h02 = 0.0, h11 = 0.0,
        h12 = 0.0, h22 = 0.0;
  int n           = 0;
...
void eval_multi_multi_UBspline_3d_s_cuda(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                         float* pos_d, float* vals_d, int num)
{
  ...
  eval_multi_multi_UBspline_3d_s_kernel<<<dimGrid, dimBlock>>>(pos_d, spline->gridInv,
                                                               spline->coefs, vals_d, spline->dim,
...
void eval_multi_multi_UBspline_3d_s_sign_cuda(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                              float* pos_d, float* sign_d, float* vals_d, int num)
{
  dim3 dimBlock(SPLINE_BLOCK_SIZE);
  dim3 dimGrid(spline->num_splines / SPLINE_BLOCK_SIZE, num);
  if (spline->num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_sign_kernel<<<dimGrid, dimBlock>>>(pos_d, sign_d, spline->gridInv,
                                                                    spline->coefs, vals_d,
...
void eval_multi_multi_UBspline_3d_s_sign_cudasplit(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                                   float* pos_d, float* sign_d, float* vals_d,
                                                   ...
                                                   cudaStream_t s)
{
  ...
  if (num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_sign_kernel<<<dimGrid, dimBlock, 0, s>>>(pos_d, sign_d,
                                                                          spline->gridInv, coefs,
                                                                          vals_d, spline->dim,
                                                                          spline->stride, num_splines,
                                                                          device_nr * num_splines *
                                                                              num);
}


void eval_multi_multi_UBspline_3d_s_vgh_cuda(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                             ...
                                             float* hess_d, int num)
{
  ...
  dim3 dimGrid(spline->num_splines / SPLINE_BLOCK_SIZE, num);
  if (spline->num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_vgh_kernel<<<dimGrid, dimBlock>>>(pos_d, spline->gridInv,
                                                                   spline->coefs, vals_d, grads_d,
                                                                   hess_d, spline->dim,
                                                                   spline->stride,
                                                                   spline->num_splines);
  cudaDeviceSynchronize();
...
__global__ static void
eval_multi_multi_UBspline_3d_s_vgl_kernel(float* pos, float3 drInv, const float* coefs, float* Linv,
...
void eval_multi_multi_UBspline_3d_s_vgl_cuda(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                             float* pos_d, float* Linv_d, float* vals_d,
                                             float* grad_lapl_d, int num, int row_stride)
{
  dim3 dimBlock(SPLINE_BLOCK_SIZE);
  dim3 dimGrid(spline->num_splines / SPLINE_BLOCK_SIZE, num);
  if (spline->num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_vgl_kernel<<<dimGrid, dimBlock>>>(pos_d, spline->gridInv,
                                                                   spline->coefs, Linv_d, vals_d,
                                                                   grad_lapl_d, spline->dim,
                                                                   spline->stride,
...
__global__ static void
eval_multi_multi_UBspline_3d_s_vgl_sign_kernel(float* pos, float sign[], float3 drInv,
                                               const float* coefs, float* Linv, float* vals,
                                               float* grad_lapl, uint3 dim, uint3 strides, int N,
                                               int row_stride, int spline_offset)
{
  int block = blockIdx.x;
  int thr   = threadIdx.x;
  int ir    = blockIdx.y;
...
void eval_multi_multi_UBspline_3d_s_vgl_sign_cuda(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                                  float* pos_d, float* sign_d, float* Linv_d,
                                                  float* vals_d, float* grad_lapl_d, int num,
                                                  int row_stride)
{
  dim3 dimBlock(SPLINE_BLOCK_SIZE);
  dim3 dimGrid(spline->num_splines / SPLINE_BLOCK_SIZE, num);
  if (spline->num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_vgl_sign_kernel<<<dimGrid, dimBlock>>>(pos_d, sign_d,
                                                                        spline->gridInv,
                                                                        spline->coefs, Linv_d,
                                                                        vals_d, grad_lapl_d,
                                                                        spline->dim, spline->stride,
...
extern "C" void
eval_multi_multi_UBspline_3d_s_vgl_sign_cudasplit(const multi_UBspline_3d_s<Devices::CUDA>* spline,
                                                  float* pos_d, float* sign_d, float* Linv_d,
                                                  float* vals_d, float* grad_lapl_d, int num,
                                                  int row_stride, float* coefs, int device_nr,
                                                  cudaStream_t s)
{
  int num_splines = spline->num_split_splines;
  dim3 dimBlock(SPLINE_BLOCK_SIZE);
  dim3 dimGrid(num_splines / SPLINE_BLOCK_SIZE, num);
  if (num_splines % SPLINE_BLOCK_SIZE) dimGrid.x++;
  eval_multi_multi_UBspline_3d_s_vgl_sign_kernel<<<dimGrid, dimBlock, 0, s>>>(pos_d, sign_d,
                                                                              spline->gridInv,
                                                                              coefs, Linv_d, vals_d,
                                                                              grad_lapl_d,
                                                                              spline->dim,
