...
template<typename T>
inline void TGemV(int n, int m, T alpha, const T* restrict amat, int lda, const T* x, int incx,
                  T beta, T* y, int incy, cudaStream_t stream);

template<>
inline void TGemV(int n, int m, double alpha, const double* restrict amat, int lda, const double* x,
                  int incx, double beta, double* y, int incy, cudaStream_t stream)
{
  ...
  cublasStatus_t err = cublasDgemv(HandleToCublas::get(),
                                   ...
                                   y,
                                   incy);
  if (err != cublasStatus_t::CUBLAS_STATUS_SUCCESS) throw std::runtime_error("cublasDgemv failed");
}
...
inline void Ger(int m, int n, double alpha, const double* x, int incx, const double* y, int incy,
                double* a, int lda, cudaStream_t stream)
{
  cublasSetStream(HandleToCublas::get(), stream);
  cublasStatus_t err = cublasDger(HandleToCublas::get(), m, n, &alpha, x, incx, y, incy, a, lda);
}
...
struct LinAlgCUDA : public LinAlgCPU
{
  ...
  inline void updateRow(Matrix<T>& psiMinv, aligned_vector<T>& psiV, int m, int lda, int rowchanged,
                        RT c_ratio_in, size_t cuda_buffer_size, void* cuda_buffer,
                        PinnedHostBuffer& host_buffer, cudaStream_t stream)
  {
    constexpr T cone(1);
    constexpr T czero(0);
    T temp[m];
    ...
    //host_buffer.toNormalTcpy(psiV.data(), psiMinv.size(), m)
    cudaStreamSynchronize(stream);
