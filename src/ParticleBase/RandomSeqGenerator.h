...
{
  OHMMS_PRECISION_FULL slightly_less_than_one =
      1.0 - std::numeric_limits<OHMMS_PRECISION_FULL>::epsilon();
  int nm1 = n - 1;
...
template <typename T> inline void makeGaussRandom(ParticleAttrib<T> &a)
{
...
template <typename T> inline void makeUniformRandom(ParticleAttrib<T> &a)
{
...
inline void makeGaussRandomWithEngine(ParticleAttrib<TinyVector<T, D>> &a,
                                      RG &rng)
{
