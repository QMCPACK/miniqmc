 ...
 * @brief Declaration of DiracDeterminantBase with a
...
public:
  bool Optimizable;
  void registerTimers();
  NewTimer UpdateTimer, RatioTimer, InverseTimer, BufferTimer, SPOVTimer,
      SPOVGLTimer;
  ...
  /** set the index of the first particle in the determinant and reset the size
   *of the determinant
  ...
  virtual void evaluateDerivatives(ParticleSet &P,
                                   const opt_variables_type &active, int offset,
                                   Matrix<RealType> &dlogpsi,
                                   Array<GradType, 3> &dG, Matrix<RealType> &dL)
  {
  }
  ...
  virtual RealType updateBuffer(ParticleSet &P, PooledData<RealType> &buf,
   ...
   */
  virtual void evaluateRatios(VirtualParticleSet &VP,
                              std::vector<ValueType> &ratios);

  virtual ValueType alternateRatio(ParticleSet &P) { return 1.0; }
  ...
  virtual ValueType evaluate(ParticleSet &P, ParticleSet::ParticleGradient_t &G,
                             ParticleSet::ParticleLaplacian_t &L);
  ...
  int WorkingIndex;
  /// a set of single-particle orbitals used to fill in the  values of the
  /// matrix
  SPOSetBasePtr Phi;
