...
//                    Urbana-Champaign Jeongnim Kim, jeongnim.kim@gmail.com,
//                    University of Illinois at Urbana-Champaign Raymond Clay
//                    III, j.k.rofling@gmail.com, Lawrence Livermore National
//                    Laboratory Ye Luo, yeluo@anl.gov, Argonne National
//                    Laboratory Mark A. Berrill, berrillma@ornl.gov, Oak Ridge
//                    National Laboratory Jeongnim Kim, jeongnim.kim@intel.com,
//                    Intel Corp.
//
      ...
      SPOVGLTimer("DiracDeterminantBase::spovgl", timer_level_fine)
{
  ...
  // BufferMode=0 will only save the inverse. Better to Buffer dpsi and d2psi
  // for common problems.  Set to 0, if memory becomes the can-do/cannot issue.
  BufferMode = 1;
}
...
DiracDeterminantBase &DiracDeterminantBase::
operator=(const DiracDeterminantBase &s)
{
  NP = 0;
...
/** set the index of the first particle in the determinant and reset the size of
 *the determinant
...
void DiracDeterminantBase::invertPsiM(const ValueMatrix_t &logdetT,
                                      ValueMatrix_t &invMat)
{
  InverseTimer.start();
  simd::transpose(logdetT.data(), NumOrbitals, logdetT.cols(), invMat.data(),
                  NumOrbitals, invMat.cols());
...
DiracDeterminantBase::GradType DiracDeterminantBase::evalGrad(ParticleSet &P,
                                                              int iat)
{
  WorkingIndex = iat - FirstIndex;
  RatioTimer.start();
  DiracDeterminantBase::GradType g =
      simd::dot(psiM[WorkingIndex], dpsiM[WorkingIndex], NumOrbitals);
  RatioTimer.stop();
  return g;
}
 ...
 */
void DiracDeterminantBase::restore(int iat) { curRatio = 1.0; }
                                            ...
                                            ParticleSet::ParticleLaplacian_t &L)
{
  ...
  { // need to compute dpsiM and d2psim. Use Phi->t_logpsi. Do not touch psiM!
    SPOVGLTimer.start();
    Phi->evaluate_notranspose(P, FirstIndex, LastIndex, psiM_temp, dpsiM,
                              d2psiM);
    SPOVGLTimer.stop();
  }
...
void DiracDeterminantBase::copyFromBuffer(ParticleSet &P,
 ...
 */
DiracDeterminantBase::ValueType DiracDeterminantBase::ratio(ParticleSet &P,
                                                            int iat)
{
...
void DiracDeterminantBase::evaluateRatios(VirtualParticleSet &VP,
...
void DiracDeterminantBase::get_ratios(ParticleSet &P,
                                      std::vector<ValueType> &ratios)
{
  SPOVTimer.start();
...
DiracDeterminantBase::GradType DiracDeterminantBase::evalGradSourcep(
    ...
    TinyVector<ParticleSet::ParticleLaplacian_t, OHMMS_DIM> &lapl_grad)
{
  ...
  for (int ptcl = 0; ptcl < NumPtcls; ptcl++)
  {
    ...
    for (int el_dim = 0; el_dim < OHMMS_DIM; el_dim++)
    {
      ...
      {
        Grad_psi_over_psi[el_dim] +=
            Grad_phi(ptcl, orbital)[el_dim] * psiM(ptcl, orbital);
        if (el_dim == 0)
      ...
      for (int dim = 0; dim < OHMMS_DIM; dim++)
      {
        ...
        for (int orbital = 0; orbital < NumOrbitals; orbital++)
        {
          one_row_change(dim, el_dim) +=
              Grad_phi_alpha(ptcl, orbital)(dim, el_dim) * psiM(ptcl, orbital);
          if (el_dim == 0)
            one_row_change_l[dim] +=
                Grad2_phi_alpha(ptcl, orbital)[dim] * psiM(ptcl, orbital);
        }
        for (int ptcl2 = 0; ptcl2 < NumPtcls; ptcl2++)
        {
          if (ptcl != ptcl2)
          {
            ...
            for (int orbital = 0; orbital < NumOrbitals; orbital++)
            {
              ...
              toDet(0, 1) +=
                  Grad_phi(ptcl, orbital)[el_dim] * psiM(ptcl2, orbital);
              toDet_l(0, 1) += Grad2_phi(ptcl, orbital) * psiM(ptcl2, orbital);
              toDet(1, 0) +=
                  Phi_alpha(ptcl2, orbital)[dim] * psiM(ptcl, orbital);
              toDet_l(1, 0) +=
                  Phi_alpha(ptcl2, orbital)[dim] * psiM(ptcl, orbital);
              toDet(1, 1) +=
                  Phi_alpha(ptcl2, orbital)[dim] * psiM(ptcl2, orbital);
              toDet_l(1, 1) +=
                  Phi_alpha(ptcl2, orbital)[dim] * psiM(ptcl2, orbital);
            }
            two_row_change(dim, el_dim) +=
                toDet(0, 0) * toDet(1, 1) - toDet(1, 0) * toDet(0, 1);
            if (el_dim == 0)
              two_row_change_l[dim] +=
                  toDet_l(0, 0) * toDet_l(1, 1) - toDet_l(1, 0) * toDet_l(0, 1);
          }
        }
        Grad_psi_alpha_over_psi(dim, el_dim) =
            one_row_change(dim, el_dim) + two_row_change(dim, el_dim);
        outfile << Grad_psi_alpha_over_psi(dim, el_dim) << std::endl;
        grad_grad[dim][ptcl][el_dim] =
            one_row_change(dim, el_dim) + two_row_change(dim, el_dim) -
    ...
    for (int dim = 0; dim < OHMMS_DIM; dim++)
    {
      ...
      lapl_grad[dim][ptcl] += one_row_change_l[dim] + two_row_change_l[dim] -
                              Psi_alpha_over_psi[dim] * Grad2_psi_over_psi;
      for (int el_dim = 0; el_dim < OHMMS_DIM; el_dim++)
      {
        lapl_grad[dim][ptcl] -= (RealType)2.0 *
                                Grad_psi_alpha_over_psi(dim, el_dim) *
                                Grad_psi_over_psi[el_dim];
        lapl_grad[dim][ptcl] +=
            (RealType)2.0 * Psi_alpha_over_psi[dim] *
            (Grad_psi_over_psi[el_dim] * Grad_psi_over_psi[el_dim]);
      }
    }
  }
  outfile.close();
...
void DiracDeterminantBase::evaluateHessian(ParticleSet &P,
...
DiracDeterminantBase::GradType DiracDeterminantBase::evalGradSource(
    ParticleSet &P, ParticleSet &source, int iat,
    TinyVector<ParticleSet::ParticleGradient_t, OHMMS_DIM> &grad_grad,
    TinyVector<ParticleSet::ParticleLaplacian_t, OHMMS_DIM> &lapl_grad)
{
  ...
  for (int i = 0, iel = FirstIndex; i < NumPtcls; i++, iel++)
  {
    ...
    for (int dim = 0; dim < OHMMS_DIM; dim++)
    {
      ...
      for (int j = 0; j < NumOrbitals; j++)
      {
        ...
        if (j == i)
          for (int dim_el = 0; dim_el < OHMMS_DIM; dim_el++)
            lapl_grad[dim][iel] -= (RealType)2.0 *
                                   grad_phi_alpha_Minv(j, i)(dim, dim_el) *
...
void DiracDeterminantBase::recompute(ParticleSet &P)
{
  SPOVGLTimer.start();
  Phi->evaluate_notranspose(P, FirstIndex, LastIndex, psiM_temp, dpsiM, d2psiM);
  SPOVGLTimer.stop();
...
void DiracDeterminantBase::evaluateDerivatives(
...
DiracDeterminantBase::DiracDeterminantBase(const DiracDeterminantBase &s)
    : OrbitalBase(s), NP(0), Phi(s.Phi), FirstIndex(s.FirstIndex),
      UpdateTimer(s.UpdateTimer), RatioTimer(s.RatioTimer),
      InverseTimer(s.InverseTimer), BufferTimer(s.BufferTimer),
      SPOVTimer(s.SPOVTimer), SPOVGLTimer(s.SPOVGLTimer)
{
  registerTimers();
