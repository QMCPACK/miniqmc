...
inline void Xgetri(int n, float *restrict a, int lda, int *restrict piv,
...
inline void Xgetrf(int n, int m, std::complex<float> *restrict a, int lda,
                   int *restrict piv)
{
  int status;
...
inline void Xgetri(int n, std::complex<float> *restrict a, int lda,
...
inline void Xgetri(int n, double *restrict a, int lda, int *restrict piv,
...
inline void Xgetrf(int n, int m, std::complex<double> *restrict a, int lda,
                   int *restrict piv)
{
  int status;
...
inline void Xgetri(int n, std::complex<double> *restrict a, int lda,
...
inline void TansposeSquare(const TIN *restrict in, TOUT *restrict out, size_t n,
                           size_t lda)
{
#pragma omp simd
...
template <typename T>
inline T computeLogDet(const T *restrict X, int n, int lda,
                       const int *restrict pivot, T &phase)
{
  T logdet(0);
  int sign_det = 1;
  for (size_t i = 0; i < n; i++)
...
template <typename T>
inline T computeLogDet(const std::complex<T> *restrict X, int n, int lda,
                       const int *restrict pivot, T &phase)
{
  T logdet(0);
  phase = T(0);
...
template <typename T> struct DiracMatrix
{
