...
extern "C"
{
  ...
  void dgetri(const int& n, double* a, const int& n0, int* piv, double* work, const int& lwork, int& st);
  void sgetri(const int& n, float* a, const int& n0, int* piv, float* work, const int& lwork, int& st);
...
void elementWiseCopy(ViewType1 destination,
                     ViewType2 source,
...
template<class ViewType1, class ViewType2>
void elementWiseCopyTrans(ViewType1 destination,
                          ViewType2 source,
                          typename std::enable_if<ViewType1::rank == 2>::type* = 0,
                          typename std::enable_if<ViewType2::rank == 2>::type* = 0)
{
...
{
  static_assert(std::is_same<arrayLayout, Kokkos::LayoutLeft>::value,
                "Require LayoutLeft Views for the time being to interface with linear algebra "
                "libraries");

#ifdef KOKKOS_ENABLE_CUDA
  static_assert(std::is_same<memorySpace, Kokkos::HostSpace>::value ||
                    std::is_same<memorySpace, Kokkos::CudaSpace>::value ||
                    std::is_same<memorySpace, Kokkos::CudaUVMSpace>::value,
                "Currently only know about HostSpace, CudaSpace and CudaUVMSpace views");
...
#else
  static_assert(std::is_same<memorySpace, Kokkos::HostSpace>::value,
                "For this build of Kokkos, currently understand HostSpace Views, maybe try "
                "building with cuda?");
  static_assert(std::is_same<valueType, float>::value || std::is_same<valueType, double>::value ||
                    std::is_same<valueType, Kokkos::complex<float>>::value ||
                    std::is_same<valueType, std::complex<float>>::value ||
...
public:
  ...
  void getri(viewType view)
  {
    ...
    */
    ...
    getri_cpu_impl(ext,
                   pointerConverter(view.data()),
                   ext,
                   piv.data(),
                   pointerConverter(work.data()),
                   work.extent(0),
                   status);
  }
  ...
  void gemvTrans(viewType A, arrType x, arrType y, valueType alpha, valueType beta)
  {
    gemv_cpu_impl('T',
                  ...
                  A.extent(1),
                  alpha,
                  pointerConverter(A.data()),
                  A.extent(0),
                  pointerConverter(x.data()),
                  1,
                  beta,
                  pointerConverter(y.data()),
                  1);
  }
  ...
  void gemmNN(viewType A, viewType B, viewType C, valueType alpha, valueType beta)
  {
    gemm_cpu_impl('N',
                  ...
                  B.extent(1),
                  A.extent(1),
                  pointerConverter(&alpha),
                  pointerConverter(A.data()),
                  A.extent(0),
                  pointerConverter(B.data()),
                  B.extent(0),
                  pointerConverter(&beta),
                  pointerConverter(C.data()),
                  C.extent(0));
  }
  ...
  void copyChangedRow(int rowchanged, viewType pinv, arrType rcopy)
  {
    ...
    {
      Kokkos::parallel_for(arrPolicyParallel_t(0, rcopy.extent(0)),
                           KOKKOS_LAMBDA(int i) { rcopy(i) = pinv(rowchanged, i); });
    }
    else
    {
      Kokkos::parallel_for(arrPolicySerial_t(0, rcopy.extent(0)),
                           KOKKOS_LAMBDA(int i) { rcopy(i) = pinv(rowchanged, i); });
    }
    Kokkos::fence();
  }
  ...
  valueType updateRatio(arrType psiV, doubleViewType psiMinv, int firstIndex)
  {
    ...
    {
      Kokkos::parallel_reduce(arrPolicyParallel_t(0, psiV_.extent(0)),
    ...
    {
      Kokkos::parallel_reduce(arrPolicySerial_t(0, psiV_.extent(0)),
                              KOKKOS_LAMBDA(int i, valueType& update) {
                                update += psiV_(i) * psiMinv_(firstIndex_, i);
                              },
                              curRatio_);
  ...
  void copyBack(doubleViewType psiMsave, arrType psiV, int firstIndex)
  {
    ...
    if (psiV_.extent(0) > arrPolicySerialThreshold)
    {
      Kokkos::parallel_for(arrPolicyParallel_t(0, psiV_.extent(0)),
                           KOKKOS_LAMBDA(int i) { psiMsave_(firstIndex_, i) = psiV_(i); });
    }
    else
    {
      Kokkos::parallel_for(arrPolicySerial_t(0, psiV_.extent(0)),
                           KOKKOS_LAMBDA(int i) { psiMsave_(firstIndex_, i) = psiV_(i); });
    }
    Kokkos::fence();
  }
...
public:
  ...
  void getrf(viewType view)
  {
    ...
    getrf_gpu_impl(m,
                   ...
                   pointerConverter(view.data()),
                   m,
                   pointerConverter(workspace.data()),
                   piv.data(),
                   info.data(),
                   cusolver_handle);
  }
  ...
  void getri(viewType view)
  {
    ...
    Kokkos::parallel_for(n, KOKKOS_LAMBDA(int i) { outView(i, i) = 1.0; });
    getri_gpu_impl(n,
                   pointerConverter(view.data()),
                   piv.data(),
                   pointerConverter(outView.data()),
                   info.data(),
                   cusolver_handle);
  ...
  void copyChangedRow(int rowchanged, viewType pinv, arrType rcopy)
  {
    ...
    Kokkos::parallel_for(rcopy_.extent(0), KOKKOS_LAMBDA(int i) { rcopy_(i) = pinv_(rowchanged, i); });
    Kokkos::fence();
  }
  template<typename doubleViewType>
  valueType updateRatio(arrType psiV, doubleViewType psiMinv, int firstIndex)
...
void checkDiff(viewType1 a, viewType2 b, const std::string& tag)
{
  ...
  Kokkos::parallel_reduce(dim0 * dim1,
                          KOKKOS_LAMBDA(int ii, vt& update) {
                            int i = ii / dim0;
                            int j = ii % dim0;
...
extern template class DeterminantDeviceImp<Devices::KOKKOS>;
