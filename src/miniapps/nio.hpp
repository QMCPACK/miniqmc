...
template <typename J2Type> void buildJ2(J2Type &J2, double rcut) {
  ...
  { // add uu/dd
    std::vector<RealType> Y = {0.28622356,    0.1947736865,  0.1319544873,
                               0.08893394669, 0.05695575776, 0.03565958405,
                               0.0220695026,  0.01296086466, 0.006601006996,
                               0.00278714433};
    std::string suu("uu");
    Func *f = new Func;
  ...
  { // add ud/du
    std::vector<RealType> Y = {0.3689309537,  0.2226722029,  0.1484296802,
                               0.09617039126, 0.0591878654,  0.03660855878,
                               0.02262411664, 0.01322279598, 0.006736329049,
                               0.002871931038};
    std::string suu("ud");
    Func *f = new Func;
    f->initialize(npts, X, Y, 0.0, rcut, suu, optimize);
...
template <typename J1Type> void buildJ1(J1Type &J1, double rcut) {
  ...
  std::vector<RealType> Y = {-0.2249112633,  -0.1847494689, -0.115481408,
                             -0.04000122947, 0.01731711068, 0.05360131926,
                             0.05983040879,  0.03955999983, 0.0173998007,
                             0.005162164083};
  std::string suu("uu");
  Func *f = new Func;
...
template <typename J1Type> void buildJ1(J1Type &J1, double rcut) {
  ...
  std::vector<RealType> Y = {-1.64485534,    -1.470658909,   -1.078893976,
                             -0.6878964509,  -0.3907004509,  -0.1962103494,
                             -0.08512755539, -0.02752356864, -0.00401798318,
                             0.0007665934444};
  std::string suu("uu");
  Func *f = new Func;
  f->initialize(npts, X, Y, -0.25, rcut, suu, optimize);
