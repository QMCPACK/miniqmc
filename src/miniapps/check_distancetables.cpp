...
int main(int argc, char **argv)
{
  ...
  for (int iel = 0; iel < nels; ++iel)
    for (int jel = iel + 1; jel < nels; ++jel)
    {
      ...
      RealType dsym =
          std::abs(d_ee->Distances[jel][iel] - d_ee->Distances[iel][jel]);
      PosType dr =
          d_ee->Displacements[jel][iel] + d_ee->Displacements[iel][jel];
  ...
  cout << "---------------------------------" << endl;
  cout << "AA SoA(upper) - SoA(lower) (ALL) distances     = "
       << sym_all_err / nn << endl;
  cout << "AA SoA(upper) + SoA(lower) (ALL) displacements = "
       << sym_disp_all_err / nn << endl;
  cout << "AA SoA(upper) - SoA(lower) distances     = " << sym_err / nn << endl;
  cout << "AA SoA(upper) + SoA(lower) displacements = " << sym_disp_err / nn
       << endl;
  ...
  std::cout << "WignerSeitzRadius " << d_ie->Rmax << std::endl;
  {
    ...
    cout << "Done with the sweep. Diffusion |els.R-R0|^2/nels = "
         << r_err / nels << endl;
  }
  {
    double r_err = 0.0;
    for (int iat = 0; iat < nels; ++iat)
    {
      ...
      r_err += r;
      if (r > eps)
        cerr << "SoA-AoS error " << iat << " " << els.RSoA[iat] << " "
             << els_aos.R[iat] << endl;
    }
    cout << "Done with the sweep. |els.R-els_aos.R|^2/nels = " << r_err / nels
         << endl;
  }
