...
#pragma omp parallel reduction(+ : t_compute, t_ratio, t_accept, naccepted)
  ...
  cout << "determinant " << nels << " Total accepted " << naccepted << " /"
       << nels * nsteps << " " << naccepted / static_cast<double>(nels * nsteps)
       << endl;
  cout << "Total recompute " << t_compute << " ratio " << t_ratio << " accept "
       << t_accept << endl;
  cout << "Per   recompute " << t_compute / (nsteps / nsubsteps) << " ratio "
       << t_ratio / (nsteps * nels) << " accept " << t_accept / naccepted
       << endl;
  // t_diffusion*=1.0/static_cast<double>(nsteps*nsubsteps*nthreads);
  // t_pseudo   *=1.0/static_cast<double>(nsteps*nthreads);
  // cout << "#per MC step steps " << nsteps << " substeps " << nsubsteps <<
  // endl;
  // cout << "diffusion_mc " << t_diffusion << " pseudo_mc  " << t_pseudo <<
  // endl;
