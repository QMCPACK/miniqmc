...
int main(int argc, char **argv)
{
...

  {
    ...
    if (ionode)
      cout << "\nNumber of orbitals/splines = " << nels
  ...
  Timer bigClock;
  bigClock.restart();
#pragma omp parallel reduction(+ : t0, tInit, ratio, vgh_t, val_t, \
                               nspheremoves, dNumVGHCalls)
  {
    Timer initClock;
    initClock.restart();
...
#pragma omp parallel reduction(+ : vgh_t_loc, v_t_loc, my_vals)
    {
      ...
      const double zval =
          2.0 * static_cast<double>(nels) / static_cast<double>(nions);
      ...
      for (int mc = 0; mc < nsteps; ++mc)
      {
        ...
        for (int iel = 0; iel < nels; ++iel)
        {
          ...
          clock.restart();
          spo.evaluate_vgh_pfor(
              els.R[iel]); // internally using omp for over the blocks
...
#pragma omp master
    ...
    vgh_t +=
        vgh_t_loc /
        (nsteps *
         ncrews); // time accumulated for all the threads for each time step.
  ...
  double numSplines =
      0.5 * nptcl; // internally, these many splines are worked upon.
  double throughput_vgh =
      (nmpi * nMajorThreads * numSplines * 1.e-06) / (vgh_t); // Per vgh_t rate.
  double throughput_v =
      (nmpi * nMajorThreads * numSplines * 1e-06) / (val_t); // Per vgh_t rate.
  ///////////////

  if (ionode)
  {
    cout << "Grid size: " << nx << "x" << ny << "x" << nz
         << " and Graphite N = " << nptcl << std::fixed
         << " and Ratio = " << ratio * fac << " and SphereMoves/ion = "
         << double(nspheremoves * 4) / double(nknots_copy * nptcl);

    //    cout << "\nIterations: " << nsteps;
    cout << "\nMPI: " << nmpi << " and Threads: " << omp_get_max_threads();
    cout << "\nLevel 1 threads: " << int(nMajorThreads)
         << " and Nested threads = " << ncrews;
    cout << "\nNumber of calls to Value = " << setprecision(1) << std::fixed
         << nspheremoves << " and VGH = " << dNumVGHCalls << endl;
    cout << "\nTime per call: Value and VGH: " << setprecision(4)
         << std::scientific << val_t << "    " << vgh_t;
    cout << "\nThroughput: Value and VGH: " << setprecision(2) << std::fixed
         << throughput_v << "    " << throughput_vgh << endl;

    cout << "\nStats: MPI Threads_L1 Threads_L2 throughput_v throughput_vgh: "
         << nmpi << "    " << int(nMajorThreads) << "    " << ncrews << "    "
         << throughput_v << "    " << throughput_vgh << endl;
